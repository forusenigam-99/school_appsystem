...
class School :
    ...
    def add_student_details (self,roll,name):
        classes = input("Enter class name : ")
        
        if not classes.isidentifier():
            return "invalid class name" 
        query = f"INSERT INTO {classes} (roll, name) VALUES (%s, %s) "
        value = (roll, name)
        mysql_test.mycursor.execute(query , value)
        mysql_test.mydb.commit()
